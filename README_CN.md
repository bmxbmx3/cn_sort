...
| 1 000 000 个词 | 约 2.7 s | 单进程 + numpy lexsort；词表唯一词多时多进程更快 |

**为什么快：**
- 2 万字优先级表一次性加载，进程生命周期内常驻内存。
- `pypinyin` 结果按词缓存——实际场景中大量词共享相同汉字，重复查询被直接跳过。
- 生产者阶段跳过 jieba——词表已按 `\n` 预分隔，直接 split 等价但快数倍。
- 最终排序用 `numpy.lexsort`（C 级多键排序），替代原先多轮 Python `list.sort()`。
- 多进程流水线使用直接 `multiprocessing.Queue` 批量传输，没有 Manager 代理进程。

多进程路径在词表唯一词数量多（如真实词典场景，数万个不同词）时效果最好；高重复率数据用单进程 + 缓存更快。
